...
class WebSocketThread(Thread):
    ...
    @socketio.on('stop_simulation', namespace='/test')
    def on_stop_simulation(self, message):
        self.stop_simulation = True
        stop_point = self.env.now
        print(stop_point)
        print(f'stop_producing at {stop_point}')
    ...
    @socketio.on('restart_simulation', namespace='/test')
    def on_restart_simulation(self, message):
        self.stop_simulation= False
        restart_point = self.env.now
        print(restart_point)
        print(f'restart_time at {restart_point}')
...
class StopSimulation(Resource):
    def post(self):
        sid = request.json.get('sid')  # 클라이언트가 'sid'를 JSON 형식으로 전달한다고 가정합니다.
        if sid in websocket_threads:
            websocket_threads[sid].on_stop_simulation(None)
            return {'status': 'stopped'}
        else:
            return {'status': 'error', 'message': 'Invalid sid'}

class RestartSimulation(Resource):
    def post(self):
        sid = request.json.get('sid')  # 클라이언트가 'sid'를 JSON 형식으로 전달한다고 가정합니다.
        if sid in websocket_threads:
            websocket_threads[sid].on_restart_simulation(None)
            return {'status': 'restarted'}
        else:
            return {'status': 'error', 'message': 'Invalid sid'}


api.add_resource(StopSimulation, '/stop_simulation')
api.add_resource(RestartSimulation, '/restart_simulation')
...
def test_connect():
    ...
    thread.start()
    websocket_threads[request.sid] = thread
